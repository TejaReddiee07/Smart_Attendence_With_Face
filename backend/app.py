...
def add_student():
    try:
        ...
        semester = (form_data.get("semester") or data.get("semester", "1")).strip()

        print("DEBUG add_student data:", {
            "admission_no": admission_no,
            "name": name,
            "branch": branch,
...
def enroll_face():
    ...
    try:
        ...
        print("DEBUG enroll_face payload:", {
            "admission_no": admission_no,
            "name": name,
        ...
        try:
            ...
            print("DEBUG enroll_face base64 decode error:", e)
            return jsonify({"success": False, "error": "Invalid image data"}), 400

        nparr = np.frombuffer(img_bytes, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        if img is None:
            print("DEBUG enroll_face cv2.imdecode returned None")
            return jsonify({"success": False, "error": "Invalid image"}), 400

        rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        print("DEBUG enroll_face rgb_img:", rgb_img.shape, rgb_img.dtype)
...
def mark_attendance():
    ...
    try:
        ...
        try:
            ...
            print("DEBUG mark_attendance base64 decode error:", e)
            return jsonify({"success": False, "error": "Invalid image data"}), 400
